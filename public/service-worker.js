 ...
 * Generated on: 2025-03-07T20:00:23.941Z with prefix: matijaculjak-com
...
const CACHE_NAME = 'matijaculjak-com-v20250307';
const CONTENT_CACHE = 'matijaculjak-com-content-v20250307';
const STATIC_CACHE = 'matijaculjak-com-static-v20250307';
const IMAGE_CACHE = 'matijaculjak-com-images-v20250307';
const API_CACHE = 'matijaculjak-com-api-v20250307';
  ...
  if (PWA_ASSETS.includes(requestURL.pathname)) {
    ...
    event.respondWith(
      caches.match(requestURL.pathname)
        .then(cachedResponse => {
          if (cachedResponse) {
            swLog.info('Serving PWA asset from cache: ' + requestURL.pathname);
            return cachedResponse;
          }
          
          // Not in cache, try to fetch
          swLog.info('PWA asset not in cache, fetching: ' + requestURL.pathname);
        ...
        .catch(error => {
          ...
          if (requestURL.pathname.includes('.png')) {
            ...
            swLog.info('Trying alternate format: ' + svgPath);
  ...
  if (MANIFEST_ASSET_MAP[requestURL.pathname]) {
    ...
    event.respondWith(
      caches.match(correctPath)
        .then(cachedResponse => {
          if (cachedResponse) {
            swLog.info(`Serving redirected manifest asset from cache: ${correctPath}`);
            return cachedResponse;
          }
          
          swLog.info(`Fetching redirected manifest asset: ${correctPath}`);
      ...
      fetch(event.request)
        ...
        .catch(async () => {
          ...
          try {
            ...
            if (cachedResponse) {
              swLog.info('Returning cached version of: ' + event.request.url);
              return cachedResponse;
            }
          } catch (error) {
          ...
          if (cachedResponse) {
            ...
            const fetchPromise = fetch(event.request)
              ...
              .catch(() => {
                ...
                swLog.info('Background fetch failed, using cached version');
              });
