 ...
 * Generated on: 2025-05-02T21:53:29.630Z with prefix: matijaculjak-com
...
const CACHE_NAME = "matijaculjak-com-v20250502";
const CONTENT_CACHE = "matijaculjak-com-content-v20250502";
const STATIC_CACHE = "matijaculjak-com-static-v20250502";
const IMAGE_CACHE = "matijaculjak-com-images-v20250502";
const API_CACHE = "matijaculjak-com-api-v20250502";
...
const PWA_ASSETS = ["/manifest.json", "/favicon.svg", "/logo.svg"];
			...
			.then((cacheNames) => {
				return Promise.all(
					cacheNames
						.filter((cacheName) => {
							return (
								cacheName.startsWith("wp-graphql-astro-") &&
								![
									CACHE_NAME,
									CONTENT_CACHE,
									STATIC_CACHE,
...
self.addEventListener("fetch", (event) => {
		...
		event.respondWith(
			caches
				...
				.then((cachedResponse) => {
					...
					swLog.debug(
						"PWA asset not in cache, fetching: " + requestURL.pathname,
					);
					return fetch(getAssetUrl(requestURL.pathname), {
						mode: "no-cors",
					}).then((response) => {
				...
				.catch((error) => {
					swLog.warn(
						"Fetching PWA asset failed: " + requestURL.pathname,
						error,
					);
	...
	if (MANIFEST_ASSET_MAP[requestURL.pathname]) {
		...
		swLog.info(
			`Redirecting manifest asset request from ${requestURL.pathname} to ${correctPath}`,
		);
		...
		event.respondWith(
			caches
				...
				.then((cachedResponse) => {
					if (cachedResponse) {
						swLog.debug(
							`Serving redirected manifest asset from cache: ${correctPath}`,
						);
						return cachedResponse;
					}
	...
	if (
		...
		requestURL.pathname.includes("/graphql")
	) {
		event.respondWith(
			fetch(event.request)
				.then((response) => {
				...
				.catch(() => {
					swLog.info(
						"API request failed, falling back to cache: " + requestURL.pathname,
					);
	...
	if (event.request.mode === "navigate") {
		event.respondWith(
			fetch(event.request)
				...
				.catch(async () => {
					swLog.info(
						"Navigation fetch failed, falling back to cache for: " +
							event.request.url,
					);
	...
	if (
		STATIC_ASSETS_PATTERNS.some((pattern) => requestURL.pathname.match(pattern))
	) {
		event.respondWith(
		...
		for (const submission of submissions) {
			try {
				...
				if (response.ok) {
					...
					swLog.info(
						`Successfully synced contact form submission ${submission.id}`,
					);
				} else {
					swLog.error(
						`Failed to sync contact form submission ${submission.id}:`,
						await response.text(),
					);
				}
			} catch (err) {
				swLog.error(
					`Error syncing contact form submission ${submission.id}:`,
					err,
				);
		...
		const db = await openDB("form-data", 1, {
			upgrade(db) {
				if (!db.objectStoreNames.contains("newsletter-forms")) {
					db.createObjectStore("newsletter-forms", {
						keyPath: "id",
						autoIncrement: true,
					});
				}
		...
		for (const submission of submissions) {
			try {
				...
				if (response.ok) {
					await store.delete(submission.id);
					swLog.info(
						`Successfully synced newsletter subscription ${submission.id}`,
					);
				} else {
					swLog.error(
						`Failed to sync newsletter subscription ${submission.id}:`,
						await response.text(),
					);
				}
			} catch (err) {
				swLog.error(
					`Error syncing newsletter subscription ${submission.id}:`,
					err,
				);
				// Leave in IndexedDB for next sync attempt
			}
	...
	return new Promise((resolve, reject) => {
		...
		if (upgradeCallback) {
			request.onupgradeneeded = (event) =>
				upgradeCallback(request.result, event);
		}
	});
}
