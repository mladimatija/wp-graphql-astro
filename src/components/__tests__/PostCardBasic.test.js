...
describe("PostCard Basic Logic", () => {
	const mockPost = {
		...
		uri: "/test-post/",
		content:
			"<p>This is test content that will be stripped and truncated for the excerpt...</p>",
		featuredImage: {
			node: {
	...
	it("renders HTML with all elements", () => {
		...
		expect(html).toContain(
			`<img src="https://example.com/image.jpg" alt="Test alt text">`,
		);
		...
		expect(html).toContain(
			`<a href="/test-post/" class="read-more">continue reading</a>`,
		);
	});
