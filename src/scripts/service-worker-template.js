      ...
      caches.open(CACHE_NAME)
        .then(cache => {
          ...
          const cachePromises = PRECACHE_ASSETS.map(url => {
            ...
            console.log(`Trying to cache: ${assetUrl}`);
...
self.addEventListener('fetch', event => {
  ...
  if (PWA_ASSETS.includes(requestURL.pathname)) {
    console.log('PWA asset request:', requestURL.pathname);
    
    event.respondWith(
      caches.match(requestURL.pathname)
        .then(cachedResponse => {
          if (cachedResponse) {
            console.log('Serving PWA asset from cache:', requestURL.pathname);
            return cachedResponse;
          }
          
          // Not in cache, try to fetch
          console.log('PWA asset not in cache, fetching:', requestURL.pathname);
        ...
        .catch(error => {
          console.log('Fetching PWA asset failed:', requestURL.pathname, error);
          ...
          if (requestURL.pathname.includes('.png')) {
            ...
            console.log('Trying alternate format:', svgPath);
  ...
  if (MANIFEST_ASSET_MAP[requestURL.pathname]) {
    ...
    console.log(`Redirecting manifest asset request from ${requestURL.pathname} to ${correctPath}`);
    
    event.respondWith(
      caches.match(correctPath)
        .then(cachedResponse => {
          if (cachedResponse) {
            console.log(`Serving redirected manifest asset from cache: ${correctPath}`);
            return cachedResponse;
          }
          
          console.log(`Fetching redirected manifest asset: ${correctPath}`);
          return fetch(correctPath);
        })
        .catch(error => {
          console.error(`Failed to serve redirected manifest asset: ${correctPath}`, error);
          return new Response('Not found', {status: 404});
        })
    );
    return;
  ...
  if (requestURL.pathname.startsWith('/api/') || requestURL.pathname.includes('/graphql')) {
    event.respondWith(
      fetch(event.request)
        ...
        .catch(() => {
          console.log('API request failed, falling back to cache:', requestURL.pathname);
