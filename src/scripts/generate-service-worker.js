...
async function getCachePrefix() {
  ...
  if (
    ...
    const siteName = process.env.PUBLIC_SITE_NAME.toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/(^-|-$)/g, "");
  ...
  if (
    ...
    try {
      ...
      log.info(
        `Using hostname from PUBLIC_SITE_URL for cache prefix: ${hostname}`,
      );
      return hostname;
    } catch (e) {
  ...
  try {
    ...
    if (!fs.existsSync(templatePath)) {
      ...
      if (fs.existsSync(outputPublicPath)) {
        ...
        throw new Error(
          "Service worker template not found and no existing file to copy from",
        );
      }
    }
    ...
    let serviceWorkerContent = template
      .replace(
        /const CACHE_NAME = ['"].*?['"]/g,
        `const CACHE_NAME = '${cachePrefix}-${version}'`,
      )
      .replace(
        /const CONTENT_CACHE = ['"].*?['"]/g,
        `const CONTENT_CACHE = '${cachePrefix}-content-${version}'`,
      )
      .replace(
        /const STATIC_CACHE = ['"].*?['"]/g,
        `const STATIC_CACHE = '${cachePrefix}-static-${version}'`,
      )
      .replace(
        /const IMAGE_CACHE = ['"].*?['"]/g,
        `const IMAGE_CACHE = '${cachePrefix}-images-${version}'`,
      )
      .replace(
        /const API_CACHE = ['"].*?['"]/g,
        `const API_CACHE = '${cachePrefix}-api-${version}'`,
      );
    ...
    log.info(
      `Service worker generated with cache prefix "${cachePrefix}" in public dir`,
    );
    ...
    if (fs.existsSync(path.dirname(outputDistPath))) {
      ...
      log.info(
        `Service worker generated with cache prefix "${cachePrefix}" in dist dir`,
      );
    }
