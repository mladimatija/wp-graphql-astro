...
log.info(
	`Auth credentials: Username=${process.env.WP_APP_USERNAME ? "Set" : "Not set"}, Password=${process.env.WP_APP_PASSWORD ? "Set" : "Not set"}`,
);
...
async function generateManifest() {
	...
	const description =
		wpData?.generalSettings?.description || DEFAULT_APP_DESCRIPTION;
	...
	const manifest = {
		...
		screenshots: [
			{
				src:
					wpData?.mediaItems?.nodes?.[0]?.mediaItemUrl ||
					"https://wp.matijaculjak.com/wp-content/uploads/2016/05/cropped-matija-culjak-logo.png",
				sizes: "512x512",
				type: "image/png",
				form_factor: "wide",
			},
			{
				src:
					wpData?.mediaItems?.nodes?.[1]?.mediaItemUrl ||
					"https://wp.matijaculjak.com/wp-content/uploads/2016/05/matija-culjak-logo.png",
				sizes: "1200x628",
				type: "image/png",
			},
		],
